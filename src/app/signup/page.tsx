import Link from "next/link";
import { signup } from "./actions";

export default function LoginPage() {
  return (
    <>
      <main className="flex flex-col gap-12 justify-start items-start">
        <hgroup className="flex flex-col gap-3">
          <h1 className="text-2xl font-bold leading-none">
            Fantasy Premier League Mentat
          </h1>
          <p className="text-lg text-mute leading-none">
            <span className="text-brand font-bold">YOUR</span> Schedule Planner,{" "}
            <span className="text-brand font-bold">YOUR</span> Ratings,{" "}
            <span className="text-brand font-bold">YOUR</span> Projections
          </p>
        </hgroup>

        <form className="flex flex-col w-full gap-6">
          <div className="flex flex-col gap-1">
            <label htmlFor="email" className="text-sm text-mute">
              Email
            </label>
            <input
              id="email"
              name="email"
              type="email"
              placeholder="john@doe.com"
              required
              className="border border-border rounded-md px-3 py-2 w-full focus:outline-1 focus:outline-brand"
            />
          </div>
          <div className="flex flex-col gap-1">
            <label htmlFor="password" className="text-sm text-mute">
              Password
            </label>
            <input
              id="password"
              name="password"
              type="password"
              placeholder="12345678"
              required
              className="border border-border rounded-md px-3 py-2 w-full focus:outline-1 focus:outline-brand"
            />
          </div>
          <div className="flex flex-col gap-1">
            <label htmlFor="email" className="text-sm text-mute">
              FPL ID
            </label>
            <input
              id="fplId"
              name="fplId"
              type="text"
              placeholder="123456"
              required
              className="border border-border rounded-md px-3 py-2 w-full focus:outline-1 focus:outline-brand"
            />
          </div>
          <div className="flex flex-col w-full gap-3">
            <button
              formAction={signup}
              className="bg-brand text-background rounded-md px-4 py-2 cursor-pointer hover:bg-foreground"
            >
              Sign up
            </button>
            <p className="text-sm text-mute">
              Already have an account?{" "}
              <Link
                href="/login"
                className="text-brand inline-flex items-center gap-1"
              >
                Log in
                <svg
                  width="15"
                  height="15"
                  viewBox="0 0 15 15"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M8.14645 3.14645C8.34171 2.95118 8.65829 2.95118 8.85355 3.14645L12.8536 7.14645C13.0488 7.34171 13.0488 7.65829 12.8536 7.85355L8.85355 11.8536C8.65829 12.0488 8.34171 12.0488 8.14645 11.8536C7.95118 11.6583 7.95118 11.3417 8.14645 11.1464L11.2929 8H2.5C2.22386 8 2 7.77614 2 7.5C2 7.22386 2.22386 7 2.5 7H11.2929L8.14645 3.85355C7.95118 3.65829 7.95118 3.34171 8.14645 3.14645Z"
                    fill="currentColor"
                    fillRule="evenodd"
                    clipRule="evenodd"
                  />
                </svg>
              </Link>
            </p>
          </div>
        </form>
      </main>
    </>
  );
}
